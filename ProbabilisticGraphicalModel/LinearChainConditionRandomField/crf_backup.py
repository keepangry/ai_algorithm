...
class CRF(object):
    ...
    def fit(self, train_data):
        ...
        self.trans_reduce = self.label_num**2
        self.weights = np.ones(self.label_num**2 + self.feature_num*self.label_num)

        # train
        self.pos(self.sentences[0], 0, self.weights)

        print(self.forward(self.sentences[0], 2))
        print(self.backward(self.sentences[0], 2))
    ...
    def backward(self, sentence_vec, i):
        sentence_length = len(sentence_vec)
        if i == sentence_length - 1:
            return 1
        return self.pos(sentence_vec, i, self.weights) * self.backward(sentence_vec, i+1)

    def forward(self, sentence_vec, i):
        if i == 0:
            return 1
        return self.pos(sentence_vec, i, self.weights) * self.forward(sentence_vec, i-1)
    ...
    def pos(self, sentence_vec, i, weights):
        """
        M
        :param sentence_vec:
        :param i:
        ...
        """
        y = sentence_vec[i][-1]
        y_pre = sentence_vec[i-1][-1] if i > 0 else LABEL_START
        one = [self.label2index[y_pre] * self.label_num + self.label2index[y]]
        for feature_poses in self.status_feature.values():
            feature_tuple = self.get_feature_tuple(feature_poses, sentence_vec, i)
            one.append(self.trans_reduce + self.feature2index[feature_tuple] * self.label_num + self.label2index[y])
        one = np.array(one)
        return weights[one].sum()
